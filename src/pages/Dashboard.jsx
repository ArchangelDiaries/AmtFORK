...
export default function Dashboard() {
  ...
  async function create(e) {
    e.preventDefault();
    try { const id = await createEvent(user, f); nav(`/crat/${id}`); } catch (err) { console.error(err); toast('Couldn’t create the event.'); }
  }
  ...
  return (
    <div className="wrap">
      ...
      <div className="row" style={{ justifyContent: 'space-between', marginBottom: 14 }}>
        ...
        <button className="btn accent" onClick={() => setF(f ? null : { name: '', kind: 'park', park: 'Siar Geata', startDate: '', endDate: '' })}>{f ? 'Cancel' : 'New event'}</button>
      </div>
      {f && (
